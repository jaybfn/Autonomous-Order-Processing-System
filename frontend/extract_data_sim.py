...
@dag(
    schedule="*/1 * * * *",  # Cron expression for running every 5 minutes
...
def process_csv_and_upload_timed():
    ...
    def transform_to_json(df: pd.DataFrame):
        ...
        if df is not None and not df.empty:
            ...
            extracted_data["order_id"] = safe_str(first_row[0]) if pd.notna(first_row[0]) else None
            extracted_data["customer_id"] = safe_str(first_row[1]) if pd.notna(first_row[1]) else None
            extracted_data["order_status"] = safe_str(first_row[2]) if pd.notna(first_row[2]) else None
            extracted_data["order_purchase_timestamp"] = safe_str(first_row[3]) if pd.notna(first_row[3]) else None
            extracted_data["order_approved_at"] = safe_str(first_row[4]) if pd.notna(first_row[4]) else None
            extracted_data["order_delivered_timestamp"] = safe_str(first_row[5]) if pd.notna(first_row[5]) else None
            extracted_data["order_estimated_delivery_date"] = safe_str(first_row[6]) if pd.notna(first_row[6]) else None
            extracted_data["customer_zip_code_prefix"] = safe_str(int(first_row[7])) if pd.notna(first_row[7]) else None
            extracted_data["customer_city"] = safe_str(first_row[8]) if pd.notna(first_row[8]) else None
            extracted_data["customer_state"] = safe_str(first_row[9]) if pd.notna(first_row[9]) else None
            extracted_data["product_id"] = safe_str(first_row[10]) if pd.notna(first_row[10]) else None
            extracted_data["seller_id"] = safe_str(first_row[11]) if pd.notna(first_row[11]) else None
            ...
            extracted_data["payment_type"] = safe_str(first_row[15]) if pd.notna(first_row[15]) else None
            ...
            extracted_data["product_category_name"] = safe_str(first_row[18]) if pd.notna(first_row[18]) else None
            ...
            logging.info(f"Extracted data before JSON conversion: {extracted_data}")
    ...
    def upload_json_to_gcs(extracted_data, bucket_name):
        ...
        logging.info(f"JSON string before upload: {json_string}")
